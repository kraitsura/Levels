...
const welcomeFile = path.join(__dirname, "../../resources/welcome.md");
const appDirectoryName = "LEVELS_2";
const fileEncoding = "utf8";
const welcomeNoteFilename = "Welcome.md";
const getQuestDir = () => {
  return `${os.homedir()}/${appDirectoryName}/Quests`;
};
const getStatDir = () => {
  return `${os.homedir()}/${appDirectoryName}/Stats`;
};
const getStatFiles = async () => {
  const rootDir = getStatDir();
  await fsExtra.ensureDir(rootDir);
  const statFiles = await fsExtra.readdir(rootDir, {
    encoding: fileEncoding,
    withFileTypes: false
  });
  let statFileNames = statFiles.filter((fileName) => fileName.endsWith(".md"));
  statFileNames = statFileNames.map((stat2) => stat2.replace(".md", ""));
  const data = await readDatabase("db");
  data.stats.forEach(async (stat2) => {
    if (!statFileNames.includes(stat2.title)) {
      console.info("Stat Note not found, creating....");
      const content = await fsExtra.readFile(welcomeFile, { encoding: fileEncoding });
      await fsExtra.writeFile(`${rootDir}/${stat2.title}.md`, content, { encoding: fileEncoding });
      const generatedFileName = stat2.title + ".md";
...
const getQuestFiles = async () => {
  ...
  await fsExtra.ensureDir(rootDir);
  const questsFileNames = await fsExtra.readdir(rootDir, {
    encoding: fileEncoding,
    withFileTypes: false
  });
  const quests = questsFileNames.filter((fileName) => fileName.endsWith(".md"));
  if (lodashExports.isEmpty(quests)) {
...
const getStatInfoFromFilename = async (filename) => {
  const fileStatistics = await fsExtra.stat(`${getStatDir()}/${filename}.md`);
  const fName = filename.replace(/\.md$/, "");
  const data = readDatabase("db");
  const stats = (await data).stats;
  const found = stats.find((stat2) => stat2.title === fName);
  return {
    title: found?.title || "",
...
const getQuestInfoFromFilename = async (filename) => {
  const fileStatistics = await fsExtra.stat(`${getQuestDir()}/${filename}`);
  return {
...
const readStatFile = async (filename) => {
  const rootDir = getStatDir();
...
const writeStatFile = async (filename, content) => {
  const rootDir = getStatDir();
  console.info(`Writing Over ${filename}`);
  return fsExtra.writeFile(`${rootDir}/${filename}.md`, content, { encoding: fileEncoding });
};
const writeQuestFile = async (filename, content) => {
  const rootDir = getQuestDir();
  console.info(`Writing Over ${filename}`);
  return fsExtra.writeFile(`${rootDir}/${filename}.md`, content, { encoding: fileEncoding });
};
const createStatFile = async () => {
  const rootDir = getStatDir();
  await fsExtra.ensureDir(rootDir);
...
const deleteStatFile = async (filename) => {
  const rootDir = getStatDir();
...
const readDatabase = async (dbName) => {
  fsExtra.ensureFile(`${dbName}.json`);
  const data = await fsExtra.readFile(`${dbName}.json`, "utf-8");
...
const writeDatabase = async (dbName, data) => {
  try {
...
const statEntryDB = async (newStatTitle) => {
  const dbName = "db";
...
const statDelDB = async (statTitle) => {
  const dbName = "db";
  const data = await readDatabase(dbName);
  const statIndex = data.stats.findIndex((stat2) => stat2.title === statTitle);
  if (statIndex !== -1) {
...
electron.app.whenReady().then(() => {
  ...
  electron.ipcMain.handle("getStats", (_, ...args) => getStatFiles(...args));
  electron.ipcMain.handle("readStat", (_, ...args) => readStatFile(...args));
  electron.ipcMain.handle("writeStat", (_, ...args) => writeStatFile(...args));
  electron.ipcMain.handle("createStat", (_, ...args) => createStatFile(...args));
  electron.ipcMain.handle("deleteStat", (_, ...args) => deleteStatFile(...args));
  electron.ipcMain.handle("getQuests", (_, ...args) => getQuestFiles(...args));
  electron.ipcMain.handle("readQuest", (_, ...args) => readQuestFile(...args));
  electron.ipcMain.handle("writeQuest", (_, ...args) => writeQuestFile(...args));
  electron.ipcMain.handle("createQuest", (_, ...args) => createQuestFile(...args));
  electron.ipcMain.handle("deleteQuest", (_, ...args) => deleteQuestFile(...args));
  electron.ipcMain.handle("readDb", (_, ...args) => readDatabase(...args));
  electron.ipcMain.handle("writeDb", (_, ...args) => writeDatabase(...args));
  createWindow();
